...
// GET /api/site.json 首页配置
export async function fetchHexoConfig(): Promise<AxiosResponse<any>> {
  return request.get<HexoConfig>('/comm/site')
}

// GET /api/posts/:pageNum.json (default 1.json)  文章列表
export async function fetchPostsList(
  currentPage: number
): Promise<AxiosResponse<any>> {
  return request.post<PostList>(`/posts?${currentPage}`)
}
...
export async function fetchImplicitPageBySource(
  ...
  return request.get<Page>(`/comm/index?${source}`)
}

// GET /api/features.json
export async function fetchFeature(): Promise<AxiosResponse<any>> {
  return request.get<Post[]>('/article/features')
}
...
export async function fetchSearchIndexes(): Promise<AxiosResponse<any>> {
  return request.get<SearchIndexes[]>('/article/search')
}
...
export async function fetchAuthorPost(
  ...
  return request.get<AuthorPosts>(`/comm/blog_author?${slug}`)
}
