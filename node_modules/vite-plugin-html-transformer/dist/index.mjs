    ...
    async closeBundle() {
      ...
      await Promise.all(
        htmlFiles.map((file, index) => {
          const dest = isMpa(viteConfig) ? pages[index].filename ? path.resolve(cwd, path.dirname(pages[index].filename)) : cwd : filename ? path.resolve(cwd, path.dirname(filename)) : cwd;
          ...
          fse.move(file, path.resolve(dest, path.basename(destFile)), {
            overwrite: true
          });
        })
      );
...
function createInput({ pages = [], template = DEFAULT_TEMPLATE }, viteConfig) {
  ...
  if (isMpa(viteConfig) || pages?.length) {
    ...
    templates.forEach((temp) => {
      ...
      input[file.replace(/\.html/, "")] = path.resolve(viteConfig.root, temp);
    });
    return input;
  } else {
    const dir = path.dirname(template);
    if (ignoreDirs.includes(dir)) {
      ...
      return {
        [file.replace(/\.html/, "")]: path.resolve(viteConfig.root, template)
      };
    }
  }
}
