...
function createPlugin(userOptions = {}) {
  const { entry, template = DEFAULT_TEMPLATE, pages = [], verbose = false, filename } = userOptions;
  let viteConfig;
  let env = {};
  return {
    name: "vite:html",
    ...
    configureServer(server) {
      ...
      if (!isMpa(viteConfig)) {
        ...
        const filename2 = DEFAULT_TEMPLATE;
        _pages.push({
          filename: filename2,
          template: template2
        });
    ...
    async closeBundle() {
      ...
      const htmlFiles = await fg__default(
      ...
      await Promise.all(
        htmlFiles.map((file, index) => {
          const dest = isMpa(viteConfig) ? pages[index].filename ? path__default.resolve(cwd, path__default.dirname(pages[index].filename)) : cwd : filename ? path__default.resolve(cwd, path__default.dirname(filename)) : cwd;
          if (!resetDir && dest !== cwd) {
            fse__default.removeSync(dest);
            resetDir = true;
          }
          const destFile = isMpa(viteConfig) ? pages[index].filename ? pages[index].filename : file : filename ? filename : file;
          fse__default.move(file, path__default.resolve(dest, path__default.basename(destFile)), {
            overwrite: true
          });
        })
      );
      const htmlDirs = await fg__default(
        outputDirs.map((dir) => dir),
        { cwd: path__default.resolve(cwd), onlyDirectories: true, absolute: true }
      );
      await Promise.all(
...
function createInput({ pages = [], template = DEFAULT_TEMPLATE }, viteConfig) {
  ...
  if (isMpa(viteConfig) || pages?.length) {
    ...
    templates.forEach((temp) => {
      ...
      input[file.replace(/\.html/, "")] = path__default.resolve(viteConfig.root, temp);
    });
    return input;
  } else {
    const dir = path__default.dirname(template);
    if (ignoreDirs.includes(dir)) {
      ...
      return {
        [file.replace(/\.html/, "")]: path__default.resolve(viteConfig.root, template)
      };
    }
  }
}
async function renderHtml(html, config) {
  ...
  if (entry) {
    ...
    result = result.replace(
      bodyInjectRE,
      `<script type="module" src="${vite.normalizePath(`${entry}`)}"><\/script>
</body>`
    );
  }
  return result;
